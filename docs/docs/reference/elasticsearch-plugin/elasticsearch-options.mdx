...
### reindexOnStockMovement

<MemberInfo kind="property" type={`'always' | 'onStockStatusChange'`} default={`'always'`}  since="2.2.0"  />
...
`ElasticsearchOptions.incrementalIndexUpdates`, which governs the write itself.
### incrementalIndexUpdates

<MemberInfo kind="property" type={`boolean`} default={`false`}  since="2.2.0"  />
...
<GenerationInfo sourceFile="packages/elasticsearch-plugin/src/options.ts" sourceLine="447" packageName="@vendure-community/elasticsearch-plugin" />
...
<GenerationInfo sourceFile="packages/elasticsearch-plugin/src/options.ts" sourceLine="733" packageName="@vendure-community/elasticsearch-plugin" />
